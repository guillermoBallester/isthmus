...
Every SQL query executed through the `query` or `explain_query` tool creates a span:
...
Every MCP tool call (`list_schemas`, `describe_table`, `query`, etc.) creates a span:
