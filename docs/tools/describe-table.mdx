...
description: "Complete table analysis: columns, keys, indexes, column statistics, sample rows, and index usage."
---
...
If a [policy file](/features/policy-engine) defines column masks, sample rows are automatically masked using the same rules as `query` results. For example, with `email: redact` and `phone: partial`:
...
This ensures table analysis never leaks PII. See [Column Masking](/features/column-masking) for details.
