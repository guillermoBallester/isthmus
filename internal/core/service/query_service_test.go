...
func TestQueryService_ValidSelect(t *testing.T) {
	...
	svc := NewQueryService(domain.NewPgQueryValidator(), exec, audit.NoopAuditor{}, testLogger())
...
func TestQueryService_WithMasks(t *testing.T) {
	...
	svc := NewQueryService(domain.NewPgQueryValidator(), exec, audit.NoopAuditor{}, testLogger())
	svc.SetMasks(map[string]string{"email": "redact"})
